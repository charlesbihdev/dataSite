...
export interface StorefrontReceiptOrder {
    reference: string;
    network: string;
...
/**
 * Tier-agnostic storefront receipt. Shared by the agent (D2) and subagent (D3) shops — the only
 * difference is which store link the "buy again" / "track" buttons point at, passed as hrefs.
 */
export function StorefrontReceiptView({
    store,
    order,
    buyHref,
    trackHref,
                    ...
                    <div className="rounded-2xl border border-border bg-card p-8 text-center shadow-sm">
                        ...
                        <div className="mt-6 grid gap-2 sm:grid-cols-2">
                            <Button asChild variant="outline">
                                <Link href={buyHref}>Buy another bundle</Link>
                            </Button>
                            <Button asChild variant="outline">
                                <Link href={trackHref}>Track my orders</Link>
                            </Button>
                        </div>
