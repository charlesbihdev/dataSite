...
/**
 * Tier-agnostic storefront order-tracking page. Shared by the agent (D2) and subagent (D3) shops; the
 * search reloads the current path (window.location.pathname) so it works for either store link, and
 * the header links point at the store home passed as homeHref.
 */
export function StorefrontTrackView({
    store,
    by,
    phone,
    reference,
            ...
            <div className="min-h-screen bg-background text-foreground">
                <header className="border-b border-border bg-card/60 backdrop-blur">
                    <div className="mx-auto flex w-full max-w-2xl items-center justify-between gap-3 px-4 py-3.5 sm:px-6 sm:py-4">
                        ...
                        <Link href={homeHref} className="shrink-0 text-sm font-medium text-brand hover:underline">
                            ← Buy bundles
                        </Link>
                    </div>
                </header>
                ...
                <main className="mx-auto w-full max-w-2xl px-4 py-10 sm:px-6">
                    ...
                    >
                        ...
                        <Label htmlFor="track-input" className="flex items-center gap-1.5 text-sm font-semibold">
                            {mode === "phone" ? <Phone className="size-4 text-brand" /> : <Hash className="size-4 text-brand" />}
                            {mode === "phone" ? "Phone number" : "Order reference"}
                        </Label>
...
function NoResults({ query, mode, store }: { query: string; mode: LookupMode; store: StorefrontContact }) {
    return (
