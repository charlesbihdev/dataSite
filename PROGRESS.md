...
**Agent storefront (D2)** — public, no auth — under `routes/domain_agent_store.php`, served on the `agent_store` surface (prod domain / local `/agent-store` prefix). `StorefrontController` (namespace `Http\Controllers\Storefront`). Standalone Inertia pages (no app shell — `app.tsx` maps any `*/storefront/*` name to `layout: null`), styled in the clean marketing look from `ui_inspo` (Laravel-Boost aesthetic) adapted to our blue/white **brand** tokens (with an authorized telecom-colour exception on bundle cards/filter chips via `lib/network-brand.ts`).

**Page layout (actor-first):** storefront pages live under `resources/js/pages/agent/storefront/{buy,track,receipt}.tsx` — everything an agent owns sits inside `pages/agent/`, and the multi-page shop earns its own `storefront/` subfolder. The reusable shopping surface is extracted to the tier-agnostic `components/storefront/store-shell.tsx` (`StoreShell`: header, hero, network-filtered bundle grid, footer, checkout slot). **Recruitment lives ONLY in the agent page**, injected into the shell as slots — the shell itself carries no "become a reseller" path, so the sealed D3 subagent shop can reuse `StoreShell` with zero recruitment.

- **Buy page** (`GET /buy/{slug}` → `agent/storefront/buy`): a **reseller strip** (top, links to recruitment), store header (name/WhatsApp + **Track order** link), gradient hero, **network filter chips**, telecom-branded product cards (agent's active packages at `selling_price`) with **Buy Now → checkout dialog** (`components/storefront/checkout-dialog.tsx`, live network-mismatch warning), and a bottom **"become a sub-agent"** recruitment section. Increments `referral_clicks`. A deactivated (`store_active=false`) or suspended (`is_active=false`) store, or an unknown slug, **404s**.
...
- **Receipt** (`GET /buy/{slug}/receipt/{order}` → `agent/storefront/receipt`): order status card (awaiting/paid/failed states), reference/bundle/recipient/amount, "buy another" + **track my orders** + contact links.
- **Track orders** (`GET /buy/{slug}/track` → `agent/storefront/track`): no-account order lookup by **phone number** or **order reference** (the `DS-` code from the receipt); returns only this store's `source=storefront` orders with payment + delivery status. Reachable from the buy header and the receipt.
